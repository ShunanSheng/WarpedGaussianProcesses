...
d=@distEuclid;
delta=1;
Gamma=[0.01,0.1,0.5,0.7,0.9,1,5,10,100]';
N=size(LogGamma,1);TP=zeros(N,1);FP=zeros(N,1);

for j=1:N
	n=20;yn=rand(n,1)>0.5; % Ground truth
    t=linspace(0,hyp0.t,M)';yhat=2*ones(n,1);gamma=Gamma(j);
    for i=1:n
...
n=20;yn=rand(n,1)>0.5;yhat=zeros(n,1);
for i=1:n
    zI=SimIntData(hyp0,hyp1,meanfunc0,covfunc0,meanfunc1,covfunc1,warpfunc,K,snI,yn(i));
    yhat(i)=NLRT(zI,H0,H1,warpfunc,K,snI,sumstats,d,delta,gamma);
