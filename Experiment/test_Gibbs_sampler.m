...
Options = struct("figOpt", true, "printOpt", true, "VaryParameter", 1);
...
%% Gibbs sampler
logLambda = zeros(length(y),1);
logLambda(xP0) = testStats(ZP0, LRT);
logLambda(xP1) = testStats(ZP1, LRT);
Lambda_train = exp(-logLambda(indexTrain)/2);

p = 1./(1 + Lambda_train);
Sigma = SBLUEprep.Cov_xtrain;
mu = meanfunc(hyp_sp.mean, Xtrain);
burnin = 2000;
S = 1000;
g_samples = GibbsSampler(p, mu, Sigma,c, burnin, S);
gs = mean(g_samples, 2);
Ytrain_hat_GS = double(mean(g_samples > hyp_sp.thres, 2) > 0.5);
sum((Ytrain - Ytrain_hat_GS).^2)/250

%% GPR using Gibbs sampler estimates of g
% KXX = SBLUEprep.Cov_xtrain;
% KxX = SBLUEprep.Cov_xstar;
% m_test = meanfunc(hyp_sp.mean, Xtest);
% g_pred= m_test + KxX / KXX * (gs - hyp_sp.mean);
% Ypred_GS = double(g_pred > hyp_sp.thres);
Ypred_GS = SBLUE_pred(SBLUE,Ytrain_hat_GS); 
MSE.GS = sum((Ypred_GS-Ytest).^2)/length(Ypred_GS);
F1.GS = F1score(Ytest,Ypred_GS);
[TPR.GS, FPR.GS] = confusionMat(Ytest,Ypred_GS);
